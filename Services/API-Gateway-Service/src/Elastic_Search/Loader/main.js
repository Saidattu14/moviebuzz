...
const movies_data = require('../JsonFiles/main_data.json');
            ...
            "properties" : {
                ...
                "Reviews" : {
                    ...
                    "properties" : {
                        ...
                        "reviewer_url" : {"type" : "keyword"},
                        "full_review" :     {"type" : "keyword"},
                        "short_review" :     {"type" : "keyword"},
                        "review_date" :     {"type" : "keyword"},
                        "likesCount" :    {"type" : "integer"},
                        "dislikesCount" :    {"type" : "integer"},
                        "review_data" :    {"type" : "keyword"},
                        "rating_value" :    {"type" : "keyword"},
                    }
                }
            }, 
          }
...
const fun = async(data,doc) => {
    
    doc.Year = parseInt(doc.Year);
    doc.imdbRating = parseInt(doc.imdbRating);
    doc.imdbVotes = parseInt(doc.imdbVotes);
    let pr = new Promise(async(resolve,reject) => {
        for(let j=0;j<data.length;j++) {
            if(data[j].Name != undefined) {
                let index_name = data[j].Name.toLowerCase();
                index = crypto.createHash('md5').update(index_name).digest("hex");
                try {
                    let response = await check_index_exits(index);
                    if(response == false)
                    {
                    ...
                    await add_index_value(index,doc)
...
const indexing1 = async() => {
    ...
    {
        movies_data[i].Reviews = [];
        arr.push(fun(movies_data[i].Country,movies_data[i]));
        await Promise.all(arr);
        if(i%1000 == 0) {
           console.log(i + "Documents Added");
        }
    }
}
